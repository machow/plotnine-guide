...
* Individual geoms draws one observation (row) with one element.
* Collective geoms draws multiple observations (rows) with one element (e.g. a boxplot).
* Collective geoms include lines (since a line is drawn between observations).
...
```{python}
#| layout-nrow: 1
# TODO: can we find a better example, or a fast way to do a tidyr::complete?
p = ggplot(mpg, aes("class", fill="drv"))

p + geom_bar()
p + geom_bar(position=position_dodge(preserve="single"))
```
