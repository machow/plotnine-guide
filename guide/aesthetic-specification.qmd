...
```{python}
# | echo: false
# | fig-alt: "Point shapes and labels by the characters that represent them.
# |   The shapes fall into two categories, filled shapes and unfilled shapes."
import numpy as np
from plotnine.scales.scale_shape import shapes as filled_shapes, unfilled_shapes
...
df = pd.DataFrame({
    ...
    "y": 1.5 * np.hstack([np.arange(n1) // ncols, np.arange(n2) // ncols]),
    "shape": pd.Series(shapes, dtype=object),
    "shape_kind": np.repeat(["Filled Shapes", "Unfilled Shapes"], (n1, n2))
})


def double_quote_strings(series):  # new
    return [
...
(
    ggplot(df, aes("x", "y"))
    + geom_tile(width=0.9, height=0.9, fill="#F8F8F8")
    ...
    + geom_text(aes("x", "y-.5", label="double_quote_strings(shape)"), va="bottom", color="RoyalBlue")
    + facet_wrap("shape_kind", ncol=1)
    + scale_shape_identity()
    + scale_y_reverse(limits=(-1, 2))
    + coord_equal()
    + theme_void()
    + theme(
