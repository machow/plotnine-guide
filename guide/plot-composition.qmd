...
Use the `-` operator to put the right-hand plot group twice as much space as each individual plot on the left.
...
Notice that the top row gives each plot equal space, while the bottom row gives two plots worth of space to the plot on the right (the one added with `-`).
